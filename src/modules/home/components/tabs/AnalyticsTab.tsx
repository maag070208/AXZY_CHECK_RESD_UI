...
const StatsCard = ({ title, value, icon, loading, shades }: StatsCardProps) => {
  const shade50 = shades[50];
  const shade500 = shades[500];

  return (
    <ITCard className="p-6 shadow-lg shadow-slate-100/50 border border-slate-100 bg-white rounded-3xl relative overflow-hidden group">
      <div
        className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 rounded-full group-hover:scale-110 transition-transform duration-500"
        style={{ backgroundColor: hexToRgba(shade500, 0.1) }}
      />
      {loading && (
      ...
      <div className="relative z-10">
        <div
          className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-xl shadow-sm"
          style={{ backgroundColor: shade50, color: shade500 }}
        >
          {icon}
        </div>
        <ITText className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 block">
          {title}
        </ITText>
        <ITText className="text-3xl font-black text-slate-800 block">
          {value ?? 0}
        </ITText>
      </div>
    </ITCard>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatsCard
          title="Eventos/Incidencias"
          value={stats?.totalIncidents}
          icon={<FaExclamationTriangle />}
          loading={loading}
          shades={warningShades}
        />
        <StatsCard
          title="Puntos Escaneados"
        ...
        <StatsCard
          ...
          value={stats?.incompleteRounds}
          icon={<FaChartLine />}
          loading={loading}
          shades={primaryShades}
        />
        <StatsCard
          title="Puntos Omitidos"
          value={stats?.missedScans}
          icon={<FaTimesCircle />}
          loading={loading}
          shades={dangerShades}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <ITCard className="lg:col-span-8 shadow-lg shadow-slate-100/50 border border-slate-100 bg-white rounded-3xl p-6 relative">
          <ITText className="text-lg font-bold text-slate-800 mb-6 block">
            Top Desempeno (Escaneos)
          </ITText>
        ...
        <ITCard className="lg:col-span-4 shadow-lg shadow-slate-100/50 border border-slate-100 bg-white rounded-3xl p-6 flex flex-col items-center">
          <ITText className="text-lg font-bold text-slate-800 mb-6 text-left w-full block">
        ...
        <ITCard className="lg:col-span-12 shadow-lg shadow-slate-100/50 border border-slate-100 bg-white rounded-3xl p-6 relative">
          <div className="flex items-center justify-between mb-6">
            <ITText className="text-lg font-bold text-slate-800 block">
              Comparativa de Carga de Trabajo
            </ITText>
            <ITBadget color="primary" variant="outlined" size="small">
              Ranking Operacional
            </ITBadget>
          </div>
          ...
          <ITText className="text-[10px] text-slate-400 mt-4 italic block">
            * El indice de carga se calcula ponderando escaneos, reportes y rondas finalizadas.
          </ITText>
        </ITCard>
      </div>
    </div>
